...
PreviewController::SelectDevice(const TCHAR* pVideo, const TCHAR* pAudio, HWND hwnd, RECT* prc)
{
	...
	if (m_bAudio)
	{
		m_pSourceGraph->AddFilter(pfAudio, strAudio);
	}
...
PreviewController::SetNextFilename(const TCHAR* pFile)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		hr = pBuilder->RenderStream(NULL, &MEDIATYPE_Video, punkSource, NULL, pfMux);
		if (SUCCEEDED(hr) && m_bAudio)
		{
			hr = pBuilder->RenderStream(NULL, &MEDIATYPE_Audio, punkSource, NULL, pfMux);
		}
