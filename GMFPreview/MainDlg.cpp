...
public:
	DeviceDlg()
	: m_catid(CLSID_VideoInputDeviceCategory)
	{
	}
...
//	LRESULT NotifyHandler(int /*idCtrl*/, LPNMHDR /*pnmh*/, BOOL& /*bHandled*/)
	...
	void SetCategory(REFCLSID catid)
	{
		m_catid = catid;
	}
	void SetCaption(const TCHAR* pCaption)
	{
...
DeviceDlg::OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
{
	...
	HRESULT hr = pCreate->CreateClassEnumerator(m_catid, &pEnum, 0);
    if (hr != S_OK) {
...
{
	LRESULT idx = SendDlgItemMessage(IDC_DEVICELIST, LB_GETCURSEL, 0, 0);
	if (idx < 0)
	{
		m_strDevice = "";
	} else {
		LRESULT cch = SendDlgItemMessage(IDC_DEVICELIST, LB_GETTEXTLEN, idx, 0);
...
LRESULT CMainDlg::OnBnClickedDevice(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	{
		_bstr_t strVideo = dlg.DeviceName();
		_bstr_t strAudio;
		dlg.SetCategory(CLSID_AudioInputDeviceCategory);
		dlg.SetCaption(TEXT("Select Audio Input or Cancel for Video-only"));
		if (dlg.DoModal(m_hWnd) == IDOK)
		{
			strAudio = dlg.DeviceName();
		}
		...
		rc.right -= 15;
		rc.bottom -= 15;
		HRESULT hr = m_Previewer.SelectDevice(strVideo, strAudio, GetDlgItem(IDC_PREVIEW), &rc);
		if (hr != S_OK)
		{
			TCHAR ach[256];
